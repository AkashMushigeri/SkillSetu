    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 bg-gradient-to-r from-brand-emerald to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-bold text-xs rounded-xl shadow-md flex items-center gap-2 self-start sm:self-auto transition-all"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-card">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Total Offers Issued</span>
          <span className="text-xl font-extrabold text-slate-900 font-mono mt-1 block">{offers.length} Candidates</span>
        </div>
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-card">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Accepted Offers</span>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-card">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Pending Sign-off</span>
          <span className="text-xl font-extrabold text-amber-700 font-mono mt-1 block">
            {offers.filter((o) => o.status === 'Sent').length} Pending
          </span>
        </div>
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-card">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Avg. Placement CTC</span>
          <span className="text-xl font-extrabold text-teal-700 font-mono mt-1 block">₹13.5 LPA</span>
        </div>
      </div>
                ...
                <tr key={offer.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <span className="font-bold text-slate-900 block">{offer.candidateName}</span>
                        <span className="text-[11px] text-slate-500">{offer.candidateCollege.split(',')[0]}</span>
                      </div>
                    </div>
                  </td>

                  <td className="px-6 py-4">
                    <span className="font-semibold text-slate-900 block">{offer.roleTitle}</span>
                    <span className="text-[11px] text-teal-700 font-medium">{offer.type} · {offer.workMode}</span>
                  </td>

                  <td className="px-6 py-4">
                    <span className="font-mono font-bold text-emerald-700 block">{offer.compensation}</span>
                    <span className="text-[10px] text-slate-500">Fixed: {offer.breakdown.baseFixed}</span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      ...
                      {offer.status === 'Sent' && <Clock className="w-3 h-3 text-amber-600" />}
                      {offer.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-white hover:bg-slate-50 text-slate-700 rounded-xl font-semibold text-xs flex items-center gap-1 transition-colors border border-slate-200 shadow-xs"
                      >
                      ...
                      {offer.status === 'Sent' && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-gradient-to-r from-brand-emerald to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white rounded-xl font-bold text-xs flex items-center gap-1 shadow-md transition-colors"
                        >
      ...
      {isGenerating && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-5 animate-in fade-in zoom-in-95 text-slate-900">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                <FileCheck className="w-5 h-5 text-emerald-600" />
                <h2 className="text-lg font-bold text-slate-900">Generate Official Offer Letter</h2>
              </div>
              <button
            ...
            <form onSubmit={handleGenerateSubmit} className="space-y-3.5">
              <div>
                <label className="text-[11px] font-bold text-slate-700 block mb-1">Select Candidate</label>
                <select
                  value={candidateId}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-slate-700 block mb-1">Offer Designation</label>
                  <input
                    type="text"
                    value={roleTitle}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-slate-700 block mb-1">Total Compensation (CTC)</label>
                  <input
                    type="text"
                    value={compensation}
                    onChange={(e) => setCompensation(e.target.value)}
                    placeholder="e.g. ₹14,50,000 / annum"
                    className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs text-slate-900 focus:outline-none focus:border-emerald-500 font-mono shadow-xs"
                  />
                </div>
                <div>
                  <label className="text-[11px] font-bold text-slate-700 block mb-1">Base Fixed Component</label>
                  <input
                    type="text"
                    value={baseFixed}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-slate-700 block mb-1">Joining Date</label>
                  <input
                    type="text"
                    value={joiningDate}
