        ...
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="p-1 rounded-full bg-emerald-500/20 text-emerald-400" title="Accredited College">
                <ShieldCheck className="w-5 h-5" />
              </span>
            </div>
            <p className="text-xs text-slate-300 font-semibold">{profile.location} &bull; ID: {profile.collegeId}</p>
            <p className="text-xs text-emerald-400 font-bold">{profile.naacGrade} &bull; {profile.nirfRank}</p>
          </div>
        </div>

        <button
          onClick={() => setIsEditing(true)}
          className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl text-xs flex items-center gap-2 border border-slate-700 shadow-md"
        >
        ...
        <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="p-3 bg-slate-800/60 rounded-xl border border-slate-700/60">
              ...
              <a href={profile.website} target="_blank" rel="noreferrer" className="font-bold text-emerald-400 hover:underline">
                {profile.website}
              </a>
            </div>
            ...
            <div className="p-3 bg-slate-800/60 rounded-xl border border-slate-700/60">
              ...
              <span className="font-bold text-white">{profile.principalName}</span>
            </div>
          </div>
          ...
          <div className="p-3.5 bg-slate-800/60 rounded-xl border border-slate-700/60 text-xs">
            ...
            <span className="font-bold text-slate-200">{profile.address}</span>
          </div>

          <div className="pt-2">
            <span className="text-xs font-bold text-slate-400 block mb-2 uppercase tracking-wider">Active Academic Departments</span>
            <div className="flex flex-wrap gap-2">
              {profile.departments.map((d) => (
                <span key={d} className="px-3 py-1 bg-slate-800 text-emerald-300 font-bold text-xs rounded-xl border border-slate-700">
                  {d}
                </span>
              ))}
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-xs">
                ...
                <p className="text-slate-400 mt-1">{profile.placementOfficer.email}</p>
                <p className="text-slate-400">{profile.placementOfficer.phone}</p>
              </div>
            </div>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-3 text-xs">
            <div className="flex justify-between items-center p-2.5 bg-slate-800/60 rounded-xl">
              <span className="text-slate-400">Total Enrolled Students</span>
              <span className="font-bold text-white font-mono">{profile.totalStudents}</span>
            </div>
            <div className="flex justify-between items-center p-2.5 bg-slate-800/60 rounded-xl">
              <span className="text-slate-400">Active Industry Partners</span>
              <span className="font-bold text-emerald-400 font-mono">{profile.industryPartnersCount} Partners</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-xl w-full shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="font-bold text-white text-base">Edit College Profile</h3>
            ...
            <form onSubmit={handleSave} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Principal Name</label>
                  <input
                    type="text"
                    value={principalName}
              ...
              <div className="pt-3 border-t border-slate-800 flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl"
                >
                  Save Changes
                </button>
              </div>
