...
export const CollegeNavbar: React.FC<NavbarProps> = ({ onOpenSearch, onOpenMobileMenu }) => {
  ...
  return (
    <header className="sticky top-0 z-20 bg-white/95 border-b border-slate-200 backdrop-blur-md px-4 sm:px-6 py-3">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 max-w-xl">
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-3.5 py-2 bg-slate-50 hover:bg-slate-100/80 border border-slate-200 rounded-xl text-xs text-slate-500 transition-all group"
          >
            ...
            <kbd className="hidden sm:inline-block px-2 py-0.5 text-[10px] font-semibold text-slate-500 bg-white rounded border border-slate-200 shadow-2xs">
              ⌘K
            </kbd>
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-700">
            ...
            >
              <option value="2026–27" className="bg-white text-slate-900">AY 2026–27</option>
              <option value="2025–26" className="bg-white text-slate-900">AY 2025–26</option>
              <option value="2024–25" className="bg-white text-slate-900">AY 2024–25</option>
            </select>
          </div>
          ...
          <button
            ...
            className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 hover:text-slate-900 hover:border-slate-300 transition-all relative"
            title="Help & Tour"
          >
                ...
                <div className="max-h-80 overflow-y-auto divide-y divide-slate-100 custom-scrollbar">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      ...
                      >
                        <div className="w-2 h-2 mt-1.5 rounded-full bg-emerald-500 shrink-0 opacity-80" />
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 p-1.5 rounded-xl bg-slate-50 border border-slate-200 hover:border-slate-300 transition-all text-left"
            >
              <img
                src={profile.placementOfficer.avatar}
                alt={profile.placementOfficer.name}
                className="w-8 h-8 rounded-lg object-cover bg-slate-200 border border-slate-300 shrink-0"
              />
              <div className="hidden sm:block text-xs pr-1">
                <p className="font-bold text-slate-900 leading-tight truncate">{profile.placementOfficer.name}</p>
                <p className="text-[10px] text-emerald-600 font-semibold leading-tight truncate">
            ...
            {profileDropdownOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 p-2 divide-y divide-slate-100">
                <div className="px-3 py-2">
                  <p className="text-xs font-bold text-slate-900">{profile.placementOfficer.name}</p>
                  <p className="text-[11px] text-slate-500">{profile.placementOfficer.email}</p>
                  <span className="inline-block mt-1 px-2 py-0.5 rounded bg-emerald-50 border border-emerald-200 text-emerald-800 text-[10px] font-bold">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-900 text-base">SKILLSETU College Help</h3>
              </div>
              <button
            ...
            <button
              onClick={() => setHelpOpen(false)}
              className="w-full py-2.5 bg-gradient-to-r from-brand-teal to-brand-emerald hover:from-teal-700 hover:to-emerald-700 text-white font-bold rounded-xl text-xs shadow-md shadow-emerald-900/10"
            >
              Got it, thanks!
            </button>
          </div>
